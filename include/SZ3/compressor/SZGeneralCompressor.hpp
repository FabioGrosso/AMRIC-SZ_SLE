...
namespace SZ {
    ...
    public:
        ...
        void saveToBinaryFile(const std::vector<int>& data, const std::string& filename) {
            ...
            for (int value : data) {
                ...
                outFile.write(reinterpret_cast<const char*>(&floatValue), sizeof(float));
            }
        ...
        uchar *compress(const Config &conf, T *data, size_t &compressed_size) {
            ...
            std::cout << "encoded data size: " << buffer_pos - buffer_temp << std::endl;
        ...
        T *decompress(uchar const *cmpData, const size_t &cmpSize, T *decData) {
            size_t smallX, smallY, smallZ, bigX, bigY, bigZ;
            std::ifstream inFile("box.txt");
            if (inFile.is_open()) {
                inFile >> smallX >> smallY >> smallZ;
                inFile >> bigX >> bigY >> bigZ;
                inFile.close();
            } else {
                std::cerr << "Unable to open file for box reading\n";
            }
            ...
            // Print the length of quant_inds
            // std::cout << "Length of quant_inds 111: " << quant_inds.size() << std::endl;
            count = 0 + 32*32*smallZ;
