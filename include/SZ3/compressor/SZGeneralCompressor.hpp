...
namespace SZ {
    ...
    public:
        ...
        uchar *compress(const Config &conf, T *data, size_t &compressed_size) {
                ...
                for(int i = 0; i < maxBlk; ++i) {
                    if (c[i] > 0) {
                        ...
                        if (temp_0 - int(temp_0) == 0){
                            ...
                            auto quant_blk = frontend.compress(ts[i], x, y, z);
                            quant_inds.insert(quant_inds.end(), quant_blk.begin(), quant_blk.end());
                            // std::cout << "quant size: " << quant_inds.size() << std::endl;
                            int* meta_blk = frontend.save(buffer_pos, regCnt);
            ...
            // for (size_t i = 0; i < quant_inds.size(); ++i) 
            // {
            //     fout.write( reinterpret_cast<char*>(&quant_inds[i]) , sizeof(int));
            // }
            // fout.close();
        ...
        T *decompress(uchar const *cmpData, const size_t &cmpSize, T *decData) {
            ...
            for (size_t i = 0; i < maxBlk; i++)
            {
                if (c[i] > 0) {
                    ...
                    frontend.load(compressed_data_pos, remaining_length, 1, 1, 1);
                    count++;
                    // std::cout << "remaining_length " << count << " : "<< int(*compressed_data_pos) << std::endl;
                }
            }
            ...
            for (size_t i = 0; i < maxBlk; i++)
            {
                if (c[i] > 0) {
                    ...
                    frontend.load(ptr[count], remainArr[count], x, x, z * c[i]);
